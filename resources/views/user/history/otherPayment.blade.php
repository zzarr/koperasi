...
@section('content')
    <div class="page-header">
        <div class="page-title">
            <h3>History Simpanan Hari Raya</h3>
        </div>
    ...
    <div class="d-flex justify-content-start mb-3">
        <button class="btn btn-primary" onclick="window.open(`{{ route('user.history.other.print') }}`)">
    ...
    <script>
        $(document).ready(function() {
            var table = $('#main-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: `{{ route('user.history.other.ajax') }}`,
                    type: 'GET',
                    dataSrc: function(json) {
                        // Filter data: hanya data dengan `amount` tidak null yang akan ditampilkan
                        return json.data.filter(row => row.amount > 0);
                    },
